...
const HUB = process.env.HUB_HTTP_URL || "https://snapchain-api.neynar.com";
...
export async function GET(req: Request) {
  ...
  try {
    ...
    const [casts, likes, following, followers, userData] = await Promise.all([
      ...
      getJson(
        `${HUB}/v1/reactionsByFid?fid=${fid}&reaction_type=like&pageSize=${pageSize}`
      ).catch(() => ({ messages: [] })),
      getJson(
        `${HUB}/v1/linksByFid?fid=${fid}&link_type=follow&pageSize=${pageSize}`
      ).catch(() => ({ messages: [] })),
      getJson(
        `${HUB}/v1/linksByTargetFid?target_fid=${fid}&link_type=follow&pageSize=${pageSize}`
      ).catch(() => ({ messages: [] })),
      getJson(`${HUB}/v1/userDataByFid?fid=${fid}`).catch(() => ({
        messages: [],
      })),
    ]);
    ...
    const display =
      user["USER_DATA_TYPE_DISPLAY"] || user["2"] || user["display"] || "";
    const bio = user["USER_DATA_TYPE_BIO"] || user["3"] || user["bio"] || "";
    const fname =
      user["USER_DATA_TYPE_FNAME"] ||
      user["1"] ||
      user["fname"] ||
      user["username"] ||
      "";
